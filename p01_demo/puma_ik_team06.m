...
%         Wraping function
        if ~isnan(th1) && exist('th1now','var')
            theta_col = [th4 th5 th6]';
%             theta_col = [th1 th2 th3 th4 th5 th6]';
            theta_temp = [theta_col-2*pi, theta_col, theta_col + 2*pi];
            theta_dis = abs(theta_temp - [th4now*ones(1,3); th5now*ones(1,3); th6now*ones(1,3);]);
%             theta_dis = abs(theta_temp - [th1now*ones(1,5);th2now*ones(1,5);...
%                 th3now*ones(1,5);th4now*ones(1,5); th5now*ones(1,5); th6now*ones(1,5);]);
            [C,I] = min(theta_dis,[],2);
            for m = 1:3
                theta_all(k,m+3) = theta_temp(m,I(m));
            end
%             theta_all(k,:) = [theta_temp(1,I(1)), theta_temp(2,I(2)), theta_temp(3,I(3)), ...
%                 theta_temp(4,I(4)), theta_temp(5,I(5)), theta_temp(6,I(6))];
        end
        k = k + 1;
    end
end
