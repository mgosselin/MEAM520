...
tStep = 0.02; % The simulation's time step, in seconds.
...
% We want the PUMA to draw a vertical circle parallel to the x-z plane.

% Define the radius of the circle.
radius = 0.14; % meters

x_center = 0.32; % meters

y_offset = 0.14; % meters
z_center = 0.42; % meters
...
for i = 1:length(t)
    ...
    [phi, theta, psi] = puma_find_euler_team06(x, y, z, x_cam, y_cam, z_cam);
    if i == 1
        th1now = 0;
        th2now = 0;
        th3now = 0;
    ...
    [th1, th2, th3, th4, th5, th6] = puma_ik_team06(x, y, z, phi, theta, psi, th1now, th2now, th3now, th4now, th5now, th6now);
%     [th1, th2, th3, th4, th5, th6] = puma_ik_2_team06(x, y, z, phi, theta, psi);
    ...
    if (i == 1)
        ...
        % points later.
        hold on;
        hcam = plot3(x_cam,y_cam,z_cam,'c.','MarkerSize',25);
...
for i = 1:6
    subplot(3,2,i)
    hold on
    plot(t, theta_history(i,:)*180/pi,'b','LineWidth', 2);
    plot(t, theta_range_deg(i,upper)*ones(1,length(t)),'m','LineWidth',2);
    plot(t, theta_range_deg(i,lower)*ones(1,length(t)),'m','LineWidth',2);
